...
【安全制約】Web Searchは使用できません。googleMapsUrlを開いた・検索した・確認したと表現せず、短縮URL文字列だけで推測しません。Workerが安全に解決したresolvedGoogleMapsContextは地点特定に使えます。これがなく地点名、locationNote、memoからsegment.before/afterを十分特定できなければ、別地点を想定せずneeds_clarificationを返します。A/BでないMAINの曖昧さだけでは確認を求めず、MAINがA/Bなら十分な特定が必要です。最新の営業・道路状況を確認済みと表現しません。
