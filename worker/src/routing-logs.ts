...
export async function saveRoutingLog(db: D1Database, input: RoutingInput, result: any, errorCode?: string, errorLocationResolution?: LocationResolution): Promise<void> {
  ...
  await db.prepare(`INSERT INTO routing_evaluation_logs (id, created_at, request_id, provider, routing_policy_version,
   ...
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), new Date().toISOString(), input.requestId,
    'openrouteservice', ROUTING_POLICY_VERSION, input.condition, 'recommended', JSON.stringify(input.condition === 'local_roads' ? ['highways'] : []),
    JSON.stringify(result?.locationResolution ?? errorLocationResolution ?? {}), result?.distanceMeters ?? null, result?.durationSeconds ?? null,
