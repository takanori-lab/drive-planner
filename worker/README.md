...
通常検索では最大5候補からcanonicalな地点名、明示された都道府県、地点種別、`confidence` を照合します。
`locationNote` は住所とは限らないためfree-text検索ヒントとしてだけ扱い、市区町村・郡を自前で構造化しません。
十分な候補がない場合だけ、`locationNote` がある地点では地点名のみの検索、最後に
`/pelias/v1/search/structured` の `venue` と、明示されている場合だけ都道府県を使う検索を行います。地点ごとのGeocodingは最大3回
...
Maps URLにlabelがあればdisplay nameやqueryよりcanonicalな地点名として優先します。また、Maps解決、両地点の
fallback、Directionsはリクエスト開始時から同じdeadlineを共有し、期限後に次のfallbackを開始しません。
