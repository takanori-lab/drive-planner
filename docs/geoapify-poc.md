...
`npm test`ではmockした`fetch`だけを使用し、実Geoapify APIやquotaを消費しません。レスポンス抽出、0件、HTTPエラー、秘密情報のredaction、Markdown生成を検証します。実測はAPIキーを持つレビュー担当者が明示的に`npm run poc:geoapify`を実行してください。
